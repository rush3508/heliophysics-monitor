"""
generate_brief.py — Generate a 150–250 word analyst brief using phi3:mini.

Loads structured stats from data/processed/, retrieves relevant chunks
from the heliophysics corpus, builds a constrained prompt for phi3:mini,
...
def _build_prompt(
    ...
    """Build a constrained prompt for phi3:mini."""
    chunk_text = "\n\n".join(
...
def _generate(prompt: str) -> str:
    """Call phi3:mini via Ollama and return the response text."""
    resp = requests.post(
...
def _deterministic_fallback(stats: dict) -> str:
    """Generate a deterministic template brief when phi3:mini fails."""
